...
function addMoviesTopage(obj){
    for(x in obj){
        ...
        ele.innerHTML="<div class='single-destinations' style='display:flex;flex-direction:column;cursor:pointer;justify-content:center;align-items:center;border-radius:20px'><div class='thumb' onclick='show("+data.movieId+")'><img style='border-top-left-radius: 20px;border-top-right-radius:20px' src=img/myimgs/"+data.cardImage+" alt=''></div><div id='movieCardName' style='color:white;font-size:15px;font-weight: bold;padding:2px'>"+data.name+"</div><div id='movieCardSubTags' style='color:white'>"+data.languages+"/"+data.category+"</div><div style='font-size:20px;width:100%;text-align:center;padding:6px;font-weight:normal;border-bottom-left-radius: 20px;border-bottom-right-radius:20px;background:linear-gradient(to right,black,grey,black);color:white'>Book Ticket</div></div>"
